...
relation_queue([Path | Rest], State0) ->
    case maps:is_key(Path, maps:get(processed, State0)) of
        ...
        false ->
            State1 = State0#{processed => maps:put(
...
inspect_dependencies([Declaration | Rest], Path, State0, NewPaths0) ->
    ...
    case file:read_link_info(Checkout) of
        ...
        {error, Reason} ->
            ...
            State2 = set_relationship(Path, Name, omitted_local_checkout,
                                      State1),
            ...
            case rebar3_reltree_fs:resolve_checkout(Checkout) of
                {ok, Target} ->
                    case ensure_target(Target, Path, Name, State0) of
                        {ok, State1, New} ->
                            case maps:is_key(Target,
                                             maps:get(entries, State1)) of
                                true ->
                                    State2 = add_relation(Path, Target, Name,
                                                          Declaration,
                                                          State1),
                                    State3 = set_relationship(
                                               Path, Name, local_checkout,
                                               State2),
                                    inspect_dependencies(Rest, Path, State3,
                                                         add_new(NewPaths0,
                                                                 New));
                                false ->
                                    ...
                                    State3 = set_relationship(
                                               Path, Name,
                                               omitted_local_checkout, State2),
                                    inspect_dependencies(Rest, Path, State3,
                                                         add_new(NewPaths0, New))
                            end;
                        {invalid, State1} ->
                            State2 = relation_anomaly(Path, Target,
                                                      target_not_valid_project,
                                                      State1),
                            State3 = set_relationship(
                                       Path, Name, omitted_local_checkout,
                                       State2),
                            inspect_dependencies(Rest, Path, State3,
                                                 NewPaths0);
                        {error, State1} ->
                ...
                {error, Reason} ->
                    State1 = relation_anomaly(Path, Checkout, Reason, State0),
                    State2 = set_relationship(Path, Name,
                                              omitted_local_checkout, State1),
                    inspect_dependencies(Rest, Path, State2, NewPaths0)
            end
    end.
...
ensure_target(Target, SourcePath, Name, State0) ->
    ...
    case maps:find(Target, Entries0) of
        {ok, Entry} ->
            ...
            Source = #{kind => checkout, path => SourcePath,
                       dependency => Name},
            Entry1 = Entry#{sources => merge_sources([Source | Sources0])},
            {ok, State0#{entries => maps:put(Target, Entry1, Entries0)},
             none};
        error ->
            case maps:find(Target, maps:get(invalid, State0)) of
                ...
                error ->
                    case rebar3_reltree_config:read(Target) of
                        {ok, Facts} ->
                            Entry = #{path => Target,
                                      sources => [#{kind => checkout,
                                                    path => SourcePath,
                                                    dependency => Name}],
                                      facts => Facts},
                            {ok, State0#{entries => maps:put(Target, Entry,
                                                               Entries0)},
                             Target};
                        {error, Reason} ->
...
add_relation(Source, Target, Name, Declaration, State0) ->
    ...
    case lists:any(fun(Edge) -> maps:get(dependency, Edge) =:= Name end,
                   Existing) of
        true ->
            State0;
        false ->
            Edge = #{source => Source, target => Target,
                     dependency => Name, declaration => Declaration},
            State0#{relations => maps:put(Source, [Edge | Existing],
                                          Relations0)}
    end.

invalid_entry(Path, Reason, State0) ->
    Invalid = maps:put(Path, Reason, maps:get(invalid, State0)),
    add_warning(warning(Path, candidate_invalid, Reason),
...
relation_anomaly(Path, RelatedPath, Detail, State0) ->
    ...
    Bad = case lists:member(Key, Existing) of
              true -> Existing;
              false -> [Key | Existing]
          end,
    RelationBad = maps:put(Path, Bad, maps:get(relation_bad, State0)),
    add_warning(warning(Path, checkout_invalid,
...
finalize(State0) ->
    ...
    Allowed = fun(Path) -> Path =:= Current orelse
                            not maps:is_key(Path, RelationBad) end,
    ...
    Edges = [Edge || {_Source, EdgeList} <- maps:to_list(Relations),
                     Edge <- EdgeList,
...
adjacency(Relations, ValidPaths, Allowed, Acc0) ->
    Acc1 = lists:foldl(fun(Path, Acc) -> maps:put(Path, [], Acc) end,
                       Acc0, ValidPaths),
    lists:foldl(
              ...
              lists:foldl(
                fun(Edge, AccIn) ->
                        ...
                        case Allowed(Source) andalso Allowed(Target) of
                            true ->
                                AccA = maps:update_with(Source,
                                    fun(V) -> [Target | V] end, [Target], AccIn),
                                maps:update_with(Target,
                                    fun(V) -> [Source | V] end, [Source], AccA);
                            false -> AccIn
                        end
                end, Acc, EdgeList)
      end, Acc1, maps:to_list(Relations)).
    ...
    {New, Seen} = lists:foldl(
                    fun(Neighbour, {Queue, SeenIn}) ->
                            case maps:is_key(Neighbour, SeenIn) of
                                true -> {Queue, SeenIn};
                                false -> {[Neighbour | Queue],
                                          maps:put(Neighbour, true, SeenIn)}
                            end
                    end, {Rest, Seen0}, Neighbours),
...
incident_issues(Included, Relations, RelationBad) ->
    ...
    BadIssues = [{Path, relation_anomaly} || Path <- Included,
                ...
                fun({_Source, EdgeList}, Acc) ->
                        lists:foldl(
                          fun(Edge, AccIn) ->
                                  Source = maps:get(source, Edge),
                                  Target = maps:get(target, Edge),
                                  SourceIncluded = maps:is_key(Source,
                                                               IncludedSet),
                                  TargetIncluded = maps:is_key(Target,
                                                               IncludedSet),
                                  SourceBad = maps:is_key(Source, RelationBad),
                                  TargetBad = maps:is_key(Target, RelationBad),
                                  case (SourceIncluded orelse TargetIncluded)
                                       andalso (SourceBad orelse TargetBad) of
                                      true -> [{Source, omitted_relation} |
                                               AccIn];
                                      false -> AccIn
                                  end
                          end, Acc, EdgeList)
                end, [], maps:to_list(Relations)),
    lists:usort(BadIssues ++ Omitted).

unique_edges(Edges) ->
    lists:sort(fun edge_less/2, maps:values(
                 maps:from_list([{{maps:get(source, E), maps:get(target, E),
                                      maps:get(dependency, E)}, E} || E <- Edges]))).
...
warning(Path, Reason, Detail) ->
    #{path => rebar3_reltree_fs:canonical(Path), reason => Reason,
      detail => Detail}.
