...
enrich(Graph, Options, Request) ->
    ...
    of
        ...
        {ok, Nodes, Omitted, Warnings} ->
            ...
            case
                rebar3_reltree_rev:enrich(
                    BaseModel,
                    Edges,
                    maps:get(rev, Request, auto),
            ...
            of
                ...
                {ok, RevisionModel} ->
                    StatusInput = #{
                        nodes => maps:get(nodes, RevisionModel),
                        graph_issues => GraphIssues,
                        ...
                        {error, Reason} ->
                            [
                                rebar3_reltree_graph:warning(
                                    Path, candidate_incomplete, Reason
                                )
                                | Acc
                            ]
                    end
...
enrich_nodes(
    ...
    case enrich_candidate(Entry, RelationshipFactsForPath) of
        ...
        {error, Reason} ->
            Warning = rebar3_reltree_graph:warning(
                Path,
        ...
        true ->
            case origin_repo(maps:get(origin, Git)) of
                {ok, Repo} ->
                    ...
                    States = [
                        badge_file_state(File, Expected)
                     || File <- Files
                    ],
                    ...
                    case
                        HasReadme andalso
                            lists:all(fun(State) -> State =:= ok end, States)
                    of
                        true ->
                            #{state => ok, files => Files};
                        false ->
                            #{
                                state => badge_mismatch,
                                files => Files,
                                expected => Expected
                            }
                    end;
...
count_substring([], _Needle, Count) ->
    Count;
count_substring(Text, Needle, Count) ->
    case lists:prefix(Needle, Text) of
        true ->
            count_substring(
                lists:nthtail(length(Needle), Text),
                Needle,
                Count + 1
            );
        false ->
            count_substring(tl(Text), Needle, Count)
    end.
