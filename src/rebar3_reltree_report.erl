...
encode_utf8(Document) ->
    case unicode:characters_to_binary(Document, unicode, utf8) of
        ...
        {incomplete, Valid, Incomplete} ->
            {error, {report_encoding, {incomplete_unicode, Valid, Incomplete}}}
    end.
...
model_local_sync_at(Model) ->
    case maps:find(local_sync_at, Model) of
        ...
        {ok, _Other} ->
            {error, {clock, invalid_clock}};
        error ->
            {error, {clock, missing_local_sync_at}}
    end.
...
warnings_section(Warnings) ->
    [
        ...
        [
            [
                "- path: ",
                value(maps:get(path, Warning)),
                "; reason: ",
                display(maps:get(reason, Warning)),
                "; detail: ",
                term_value(maps:get(detail, Warning)),
                "\n"
            ]
         || Warning <- Warnings
        ],
...
nodes_section(Nodes0, Edges) ->
    Nodes = lists:sort(fun(A, B) -> maps:get(path, A) =< maps:get(path, B) end, Nodes0),
...
node_section(Node, Edges) ->
    ...
    [
        "### node: ",
        value(maps:get(name, Node)),
        "\n",
        "- path: ",
        value(Path),
        "\n",
        "- app: ",
        value(maps:get(app, Node)),
        "\n",
        "- app_src: ",
        value(maps:get(app_src, Node, none)),
        "\n",
        "- app_vsn: ",
        value(maps:get(app_vsn, Node)),
        "\n",
        "- git_head: ",
        value(maps:get(head, Node)),
        "\n",
        version_lines(maps:get(version, Node)),
        "- README.md: ",
        presence(maps:get(readme, Node), readme),
        "\n",
        "- README.zh.md: ",
        presence(maps:get(readme, Node), readme_zh),
        "\n",
        "- ci_workflow: ",
        presence_bool(maps:get(ci_workflow, Node)),
        "\n",
        "- badge_state: ",
        display(maps:get(state, maps:get(badge, Node))),
        "\n",
...
version_lines(Version) ->
    ...
    [
        ...
        value(highest_version(Highest)),
        "\n",
        "- version_line_reason: ",
        display(maps:get(reason, Version)),
        "\n",
        "- formal_tags:\n",
...
declarations_section(Node, Edges) ->
    Rows =
        case maps:find(revision_declarations, Node) of
...
relationship_for(Name, Relationships, _LocalNames) when
...
declaration_row(Declaration, local_checkout) ->
    [
        "  - name: ",
        value(declaration_name(Declaration)),
        "; declaration: ",
        term_value(Declaration),
...
revision_row(Fact) ->
    ...
    [
        "  - name: ",
        value(maps:get(name, Fact)),
        "; declaration: ",
        term_value(maps:get(declaration, Fact)),
        "; relationship: ",
        Relationship,
        "; revision_state: ",
        ...
        "\n",
        "    source_url: ",
        value(maps:get(source_url, Fact, none)),
        "\n",
        "    selector_kind: ",
        value(maps:get(selector_kind, Fact, none)),
        "\n",
        "    selector_value: ",
        value(maps:get(selector_value, Fact, none)),
        "\n",
        "    resolved_revision: ",
    ...
    [
        ...
        none_or(
            [
                [
                    "  - project: ",
                    value(relation_project(Edge, Direction)),
                    "; dependency: ",
                    value(maps:get(dependency, Edge)),
                    "\n"
                ]
             || Edge <- Edges
            ],
...
binary_text(Text) ->
    case unicode:characters_to_list(Text, utf8) of
        ...
        {incomplete, Valid, Incomplete} ->
            erlang:error({report_encoding, {incomplete_unicode, Valid, Incomplete}})
    end.
...
sort_warnings(Warnings) ->
    Unique = maps:values(
        maps:from_list([
            {{maps:get(path, W), maps:get(reason, W), term_text(maps:get(detail, W))}, W}
         || W <- Warnings
        ])
    ),
    lists:sort(
        fun(A, B) ->
            {maps:get(path, A), maps:get(reason, A), term_text(maps:get(detail, A))} =<
                {maps:get(path, B), maps:get(reason, B), term_text(maps:get(detail, B))}
        end,
        Unique
    ).
...
format_time({{Year, Month, Day}, {Hour, Minute, Second}}) ->
    rebar3_reltree_clock:format({{Year, Month, Day}, {Hour, Minute, Second}});
format_time(Other) ->
