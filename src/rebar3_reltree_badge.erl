...
run(#{project_root := ProjectRoot, mode := Mode} = Request, Options) when
    (Mode =:= check orelse Mode =:= write), is_map(Options)
->
    RunOptions = Options#{
        tag => maps:get(
            tag,
    ...
    case read_workflow(Workflow, RunOptions) of
        ...
        {present, Content} ->
            case workflow_name(Content) of
                ...
                {ok, Name} ->
                    {error, {workflow_invalid, Workflow, {name_mismatch, "master", Name}}};
                {error, Reason} ->
...
format_error({app_src_directory_read, Path, Reason}) ->
    io_lib:format(
        "unable to read app.src directory: ~ts (~ts)",
        [Path, reason_text(Reason)]
    );
format_error({app_src_read, Path, Reason}) ->
    io_lib:format(
        "unable to read app.src: ~ts (~ts)",
        [Path, reason_text(Reason)]
    );
format_error({invalid_app_src_term, Terms}) ->
    io_lib:format(
        "app.src has an invalid application term: ~ts",
        [bounded_term(Terms)]
    );
...
run_readme_mode(ProjectRoot, write, Repo, FormalTags, Policy, Options) ->
    case read_readmes(ProjectRoot, Options) of
        ...
        {ok, Files} ->
            Expected = expected(Repo, FormalTags, Policy),
            Transformed = [
                {File, transform_file(File, Expected)}
             || File <- Files
            ],
    ...
    case fs_call(read_link_info, [Path], Options) of
        ...
        {ok, #file_info{type = regular}} ->
            case fs_call(read_file, [Path], Options) of
                ...
                {ok, _Other} -> {error, {workflow_invalid, Path, non_binary_data}};
                {error, Reason} -> {error, {workflow_read, Path, Reason}}
            end;
...
parse_workflow_name_line([]) ->
    ignore;
parse_workflow_name_line([First | _]) when
    First =:= $\s;
    First =:= $\t
->
    ignore;
    ...
    case lists:prefix("name:", Line) of
        ...
        true ->
            ...
            case scalar_workflow_name(Value1) of
                {ok, Value, Style} ->
                    {ok, Value, "name:" ++ Leading, Trailing ++ Comment, Style};
                {error, _} = Error ->
                    Error
            end
    end.

leading_spaces([C | Rest]) when C =:= $\s; C =:= $\t ->
...
split_inline_comment([C | Rest], none, Acc) when C =:= $#, Rest =/= [] ->
    case Acc of
        [Previous | _] when Previous =:= $\s; Previous =:= $\t ->
    ...
    case git_call(ProjectRoot, ["rev-parse", "--verify", "HEAD"], Options) of
        {ok, Head} when is_binary(Head), byte_size(Head) > 0 ->
            ...
            of
                ...
                {error, Reason} ->
                    {error, {git_origin, ProjectRoot, sanitize_git_reason(Reason)}}
            end;
        {ok, _Other} ->
            {error, {git_repository, ProjectRoot, empty_head}};
        {error, Reason} ->
            {error, {git_repository, ProjectRoot, sanitize_git_reason(Reason)}}
    end.
...
tag_policy(Formal) ->
    ...
    Highest = [
        Tag
     || #{tag := Tag, version := Version} <- Formal,
...
expected(Repo, FormalTags, #{release := equivalent, tags := Tags}) ->
    ...
    HighestTags = [
        Tag
     || #{tag := Tag, version := Version} <- FormalTags,
        Version =:= HighestVersion
    ],
    #{
        repo => Repo,
...
cross_file_tag_check(Results, #{release := equivalent}) ->
    ...
    case lists:usort(ValidTags) of
        ...
        _ ->
            {error, [
                {maps:get(path, File), [cross_file_tag_mismatch]}
...
count_categories(Candidates, ExpectedKinds) ->
    ...
    case
        {
            lists:member(master, Kinds),
            lists:member(release, Kinds),
            lists:member(release, ExpectedKinds),
            length(Candidates) > length(ExpectedKinds)
        }
    of
        {false, _, _, _} -> [missing];
...
canonical_separator(
    [
        {First, _Kind1, _Tag1, _Text1},
        {Second, _Kind2, _Tag2, _Text2}
    ],
    Lines
) ->
    Between = lists:sublist(
        lists:nthtail(First + 1, Lines),
        Second - First - 1
    ),
...
release_tag(Candidates, Expected) ->
    ...
    case
        [
         ...
         || Tag <- maps:get(release_tags, Expected),
            Text <- ReleaseTexts,
            Text =:= release_badge_for_expected(Expected, Tag)
        ]
    of
        [Tag] -> Tag;
        _ -> none
...
replace_old_block(Lines, Candidates, Expected) ->
    ...
    InternalEol = line_eol(
        lists:nth(First + 1, Lines),
        default_eol(Lines)
    ),
    ...
    iolist_to_binary([
        render_lines([Line || {_Index, Line} <- Before]),
        Block,
...
separator_indices(Indices, Lines) ->
    Candidates = lists:zip(
        lists:sublist(Indices, length(Indices) - 1),
        tl(Indices)
    ),
...
canonical_block(Expected, InternalEol, BoundaryEol, AddTrailingBlank) ->
    ...
    Core =
        case Release of
            none ->
                [{list_to_binary(Master), BoundaryEol}];
            equivalent ->
                Tag = hd(maps:get(release_tags, Expected)),
                [
                    {list_to_binary(Master), InternalEol},
                    {<<>>, InternalEol},
                    {list_to_binary(release_badge_for_expected(Expected, Tag)), BoundaryEol}
                ];
            _ ->
                [
                    {list_to_binary(Master), InternalEol},
...
write_changes([], _Options, _Written) ->
    ok;
write_changes(
    [{_Kind, _Path, OldContent, NewContent} | Rest],
    Options,
    Written
) when OldContent =:= NewContent ->
    write_changes(Rest, Options, Written);
write_changes(
    [{Kind, Path, OldContent, NewContent} | Rest],
    Options,
    Written
) ->
...
line_eol({Body, <<>>}, Default) ->
    case
        byte_size(Body) > 0 andalso
            binary:at(Body, byte_size(Body) - 1) =:= 13
    of
        true -> <<"\r">>;
        false -> Default
    end;
...
format_warning(#{code := equivalent_formal_tags, tags := Tags}) ->
    [
        "warning: equivalent formal tags (",
        string:join(Tags, ", "),
...
sanitize_git_reason({exit, Status, _Output}) when is_integer(Status) ->
    {git_exit, Status};
sanitize_git_reason(timeout) ->
    timeout;
sanitize_git_reason(output_too_large) ->
    output_too_large;
sanitize_git_reason(git_executable_unavailable) ->
    git_executable_unavailable;
sanitize_git_reason({port_open, _}) ->
    port_open;
sanitize_git_reason(Reason) when is_atom(Reason) -> Reason;
sanitize_git_reason(_Other) ->
    git_command_failed.

format_failures(Failures) ->
    lists:join(", ", [
        io_lib:format("~ts (~p)", [Path, Categories])
