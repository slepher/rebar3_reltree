...
extract_config(Terms) when is_list(Terms) ->
    Reltree = [Term || Term <- Terms, is_tuple(Term),
                       tuple_size(Term) >= 1,
...
normalize(Context) when is_map(Context) ->
    case required_context(Context) of
        {ok, ProjectRoot0, Cwd0, Profile, BuildBase0, ConfigOptions,
             CliRoots, CliRev} ->
                            ...
                            case absolute_path(BuildBase0, ProjectRoot) of
                                {ok, BuildBase} ->
                                    normalize_configured(
                                      ProjectRoot, Profile, BuildBase,
                                      ConfigOptions, CliRoots, CliRev);
                                {error, _} = Error -> Error
                            end;
                        {error, _} = Error -> Error
                    end;
...
format_error({conflicting_roots, Path, ExistingMode, NewMode}) ->
    io_lib:format("scan root ~ts has conflicting modes ~p and ~p",
...
required_context(Context) ->
    case {maps:find(project_root, Context),
          ...
          maps:find(cli_rev, Context)} of
        {{ok, ProjectRoot}, {ok, Profile}, {ok, BuildBase},
         {ok, ConfigOptions}, {ok, CliRoots}, {ok, CliRev}} ->
            Cwd = maps:get(cwd, Context, ProjectRoot),
            {ok, ProjectRoot, Cwd, Profile, BuildBase, ConfigOptions,
             CliRoots, CliRev};
        _ ->
            {error, {invalid_context, missing, request}}
    end.

normalize_configured(ProjectRoot, Profile, BuildBase, ConfigOptions,
                     CliRoots, CliRev) ->
...
parse_tree_args([Option | _], _Roots, _Rev) when is_list(Option),
                                                 Option =/= [],
                                                 hd(Option) =:= $- ->
            ...
            case absolute_path(Path, ProjectRoot) of
                {ok, Absolute} ->
                    case lists:keyfind(Absolute, 1, Seen) of
                        false ->
                            normalize_roots(Rest, ProjectRoot,
                                             [{Absolute, Mode} | Seen],
                                             [{Absolute, Mode} | Acc]);
                        {Absolute, Mode} ->
                            normalize_roots(Rest, ProjectRoot, Seen, Acc);
                        {Absolute, ExistingMode} ->
                            {error, {conflicting_roots, Absolute,
                                     ExistingMode, Mode}}
                    end;
                {error, _} = Error ->
                    Error
    ...
    case lists:all(fun valid_config_option/1, Options) of
        ...
        true ->
            case duplicate_config_key(scan_roots, Options) of
                true -> {error, {duplicate_config, scan_roots}};
                false ->
...
duplicate_config_key(Key, Options) ->
    length([ok || Item <- Options, is_tuple(Item), tuple_size(Item) =:= 2,
                   element(1, Item) =:= Key]) > 1.
...
valid_path(Path) ->
    ...
    lists:all(fun(Char) -> is_integer(Char) andalso Char >= 0 andalso
                              Char =< 16#10ffff end, Path).

absolute_path(Path, Base) ->
    case {valid_path(Path), valid_path(Base)} of
        {true, true} ->
            Combined = case filename:pathtype(Path) of
                           absolute -> Path;
