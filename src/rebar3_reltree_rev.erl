...
-spec enrich(map(), [map()], false | auto | true, filename:filename_all(), map()) ->
    {ok, map()} | {error, term()}.
enrich(Model, Edges, Mode, Output, Options) when
    is_map(Model), is_list(Edges), is_map(Options)
->
...
prior_context(auto, Output) ->
    case read_prior(Output) of
        ...
        {error, Reason} -> {ok, #{}, [prior_warning(Output, Reason)]}
    end;
            ...
            case external_header(Line) of
                no_header ->
                    case local_header(Line) of
                        true ->
                            parse_records(Rest, CurrentPath, Entries);
                        false ->
...
local_header(Line) ->
    ...
    case lists:prefix(Prefix, Line) of
        ...
        true ->
            ...
            case
                {
                    first_substring(Tail, Marker),
                    last_substring(Tail, Local),
                    last_substring(Tail, Omitted)
                }
            of
                {NamePosition, LocalPosition, OmittedPosition} when
                    NamePosition =/= nomatch,
                ...
                MarkerPosition ->
                    case last_substring(Tail, Relationship) of
                        ...
                        Position ->
                            ...
                            case {unescape_text(Name0), Declaration, state_text(State0)} of
...
validate_prior_record(
    ...
    case
        {Path, Name, unescape_text(Url0), unescape_text(Kind0), unescape_text(Value0),
            unescape_text(Revision0), unescape_text(Observed0), unescape_text(Network0)}
    of
        {Path, Name, {ok, Url}, {ok, Kind}, {ok, Value}, {ok, Revision}, {ok, Observed},
            {ok, Network}} ->
            case
                valid_prior_fields(
                    Path,
...
valid_prior_fields(
    ...
    case
        {
            valid_state(State),
            valid_report_text(Path),
            valid_report_text(Name),
            valid_report_text(Declaration),
            state_fields_valid(
                State, Url, Kind, Value, Revision, Observed, Network
            )
        }
    of
        {true, true, true, true, true} ->
            Identity = {
                rebar3_reltree_fs:canonical(Path),
                Name,
                Url,
                selector_kind(Kind),
                selector_value(Kind, Value)
            },
            {ok, #{
                identity => Identity,
                owner =>
                ...
                state => State,
                resolved_revision => revision_value(Revision),
...
state_fields_valid(
    ...
    (valid_external_disabled_fields(
        Url,
        Kind,
        Value,
        Revision,
...
valid_selector_fields("none", "none", "none") ->
    true;
...
valid_selector_fields(_Url, _Kind, _Value) ->
    false.
...
node_items(Node, Edges) ->
    ...
    Declarations = lists:sort(
        fun declaration_less/2,
        maps:get(dependencies, Node, [])
    ),
    [
        {Owner, Name, Declaration, relationship(Name, Relationships, Owner, Edges), Node}
...
relationship(Name, _Relationships, Owner, Edges) ->
    case
        lists:any(
            fun(Edge) ->
                maps:get(source, Edge) =:= Owner andalso
                    maps:get(dependency, Edge) =:= Name
            end,
            Edges
        )
    of
...
resolve_items(
    ...
    {ok, FactsByOwner, Warnings, Reasons, Facts};
resolve_items(
    [{Owner, Name, Declaration, Relationship, Node} | Rest],
    Nodes,
    Edges,
    Mode,
    Prior,
...
base_fact(Owner, Name, Declaration, Relationship) ->
    #{
        ...
        revision_observed_at => not_performed,
...
resolve_one(Base, _Node, Nodes, Edges, Mode, Prior, Options, Cache) ->
    case maps:get(relationship, Base) of
        local_checkout ->
            Name = maps:get(name, Base),
            Owner = maps:get(owner, Base),
            case local_target_head(Owner, Name, Edges, Nodes) of
                {ok, Head} ->
                    {ok,
                        Base#{
                            revision_state => local_checkout,
                ...
                error ->
                    {ok,
                        Base#{
                            relationship => omitted_local_checkout,
                            revision_state => local_unavailable
                        },
                        none, none}
            end;
        ...
        external when Mode =:= false ->
            case classify(maps:get(declaration, Base)) of
                ...
                {ok, Source} -> {ok, source_fact(Base, Source, tracking_disabled), none, none};
                {error, _} -> {ok, Base#{revision_state => tracking_disabled}, none, none}
            end;
        external ->
            case classify(maps:get(declaration, Base)) of
                {ok, not_applicable} ->
                ...
                {error, Reason} ->
                    ...
                    {ok, Base#{revision_state => missing}, Warning, external_revision_missing};
                {ok, Source} ->
                    ...
                    case maps:find(Identity, Cache) of
                        {ok, {CachedFact, _CachedWarning, _CachedReason}} ->
                            {ok, copy_declaration(Base, CachedFact), none, none};
                        error ->
                            resolve_external_or_prior(
                                Applicable,
                                Identity,
                                Mode,
                                Prior,
    ...
    case {Mode, maps:find(Identity, Prior)} of
        {auto, {ok, PriorRecord}} ->
            case reusable(PriorRecord) of
                true ->
                    {ok, reused_fact(Applicable, PriorRecord), none, none};
                false ->
                    resolve_external(
                        Applicable,
                        Identity,
...
copy_declaration(Base, CachedFact) ->
    CachedFact#{
        declaration => maps:get(declaration, Base),
        name => maps:get(name, Base),
        owner => maps:get(owner, Base),
...
source_fact(Base, #{source_url := Url, selector := Selector}, State) ->
    Base#{
        source_url => Url,
        selector_kind => maps:get(kind, Selector),
...
identity(Fact) ->
    {
        maps:get(owner, Fact),
        name_text(maps:get(name, Fact)),
        maps:get(source_url, Fact),
        maps:get(selector_kind, Fact),
        maps:get(selector_value, Fact)
    }.
...
attempt_external1(Fact, _Identity, PriorResult, Options, Lookup) ->
    case rebar3_reltree_clock:now(Options) of
        {error, Reason} ->
            {error, {revision_clock, Reason}};
        {ok, AttemptAt} ->
            Selector = #{
                kind => maps:get(selector_kind, Fact),
                    ...
                    case stale_fallback(PriorResult) of
                        {ok, OldRevision, OldObserved} ->
                            ...
                            {ok,
                                ...
                                Warning, external_revision_stale};
                        none ->
                            Warning = revision_warning(
                                Fact,
                            ...
                            {ok,
                                ...
                                Warning, external_revision_missing}
                    end
            end
    end.
...
prior_revision(Record) ->
    case
        {
            maps:get(resolved_revision, Record, none),
            maps:get(revision_observed_at, Record, not_performed)
        }
    of
        {Revision, Observed} when
            Revision =/= none,
...
normalize_lookup(Revision) when is_binary(Revision); is_list(Revision) ->
    normalize_lookup({ok, Revision});
normalize_lookup({error, Reason}) ->
    {error, bounded_lookup_reason(Reason)};
normalize_lookup(_Other) ->
    {error, lookup_failed}.
...
bounded_lookup_reason(timeout) ->
    timeout;
bounded_lookup_reason(git_executable_unavailable) ->
    git_executable_unavailable;
bounded_lookup_reason(no_matching_revision) ->
    no_matching_revision;
bounded_lookup_reason(malformed_object_id) ->
    malformed_object_id;
bounded_lookup_reason(incompatible_revisions) ->
    incompatible_revisions;
bounded_lookup_reason({git_exit, Status}) when is_integer(Status) ->
    {git_exit, Status};
bounded_lookup_reason(_Other) ->
    lookup_failed.
...
attach_facts(Nodes, FactsByOwner) ->
    [
        Node#{
            revision_declarations => lists:reverse(
                maps:get(
                    rebar3_reltree_fs:canonical(maps:get(path, Node)),
                    FactsByOwner,
                    []
                )
            )
...
caveats(false) ->
    [
        external_revision_tracking_disabled,
        readme_mutation_not_performed
    ];
caveats(_Mode) ->
    [external_revision_read_only, readme_mutation_not_performed].
...
normalize_selector(undefined) ->
    {ok, #{kind => head, value => "HEAD"}};
normalize_selector({Kind, Value}) when
    Kind =:= branch;
    Kind =:= tag;
    Kind =:= ref
->
...
normalize_selector(_Other) ->
    {error, invalid_git_selector}.
...
source_term({git, Url}) ->
    {ok, Url, undefined};
source_term({git, Url, Selector}) ->
    {ok, Url, Selector};
source_term(Source) when
    is_tuple(Source),
    tuple_size(Source) >= 1,
    element(1, Source) =:= git
->
...
unescape_text([$\\, $n | Rest], Acc) ->
    unescape_text(Rest, [$\n | Acc]);
unescape_text([$\\, $r | Rest], Acc) ->
    unescape_text(Rest, [$\r | Acc]);
unescape_text([$\\, $\\ | Rest], Acc) ->
    unescape_text(Rest, [$\\ | Acc]);
unescape_text([$\\, $` | Rest], Acc) ->
    unescape_text(Rest, [$` | Acc]);
unescape_text([$\\, $| | Rest], Acc) ->
...
unescape_text([$\\, $] | Rest], Acc) ->
    unescape_text(Rest, [$] | Acc]);
unescape_text([$\\ | _Rest], _Acc) ->
    {error, invalid_escape};
unescape_text([Char | Rest], Acc) ->
    unescape_text(Rest, [Char | Acc]).
...
first_substring(Text, Needle) -> first_substring(Text, Needle, 0).

first_substring([], _Needle, _Position) ->
    nomatch;
...
last_substring([], _Needle, _Position, Found) ->
    Found;
