...
- `--write` 维护唯一的 `master CI` badge，并移除工具管理的 release badge。
- `--write --tag` 还会按照当前 `app.src` 版本写入 release badge。应在选择或提升版本后、创建正式 tag 前运行。
- `--check` 校验 master badge，以及指向最高可达正式 tag 的 release badge。应在创建 tag 后运行。
