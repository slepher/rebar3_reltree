...
default_request_test() ->
    ...
    ?assertEqual(
        "/workspace/project/_build/default",
        maps:get(build_base_dir, Request)
    ),
    ?assertEqual(
        "/workspace/project/_build/default/reltree/project.md",
...
cli_precedence_and_deduplication_test() ->
    ...
    ?assertEqual(
        [
            {"/workspace/first", shallow},
            {"/workspace/second", deep}
        ],
        maps:get(scan_roots, Request)
    ),
...
contradictory_modes_fail_test() ->
    ...
    ?assertMatch(
        {error, {conflicting_roots, "/workspace/one", shallow, deep}},
        rebar3_reltree_request:normalize(Context)
    ).

invalid_configured_root_fails_test() ->
    ?assertMatch(
        {error, {invalid_config, scan_roots, [{bad, root}]}},
...
empty_configured_root_fails_as_config_test() ->
    ?assertMatch(
        {error, {invalid_config, scan_roots, []}},
        rebar3_reltree_request:normalize(
            context([{scan_roots, [[]]}])
        )
    ).
...
malformed_configured_path_fails_as_config_test() ->
    ?assertMatch(
        {error, {invalid_config, scan_roots, [bad]}},
        rebar3_reltree_request:normalize(
...
malformed_deep_configured_root_fails_test() ->
    ?assertMatch(
        {error, {invalid_config, scan_roots, [{"../one", shallow}]}},
        rebar3_reltree_request:normalize(
...
invalid_cli_root_fails_test() ->
    ?assertMatch(
        {error, {invalid_option, scan_roots, ":deep"}},
        rebar3_reltree_request:normalize(
...
cli_parser_failures_test() ->
    ...
    ?assertMatch(
        {error, {invalid_option, root, "--root"}},
