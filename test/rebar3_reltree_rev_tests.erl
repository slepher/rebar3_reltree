...
auto_reuses_exact_v2_prior_and_true_does_not_test() ->
    ...
    try
        ...
        ?assertEqual(0, get_count()),
        erase(lookup_count),
        {ok, TrueModel} = rebar3_reltree_rev:enrich(
            Base,
            [],
            true,
            Output,
...
duplicate_prior_records_fold_and_reuse_twice_test() ->
    ...
    ReportModel = (model([DuplicateNode]))#{
        status => up_to_date,
...
prior_path_backslash_round_trip_test() ->
    ...
    try
        ...
        Identity = {rebar3_reltree_fs:canonical(Path), "dep", "file:///repo", head, "HEAD"},
...
stale_and_missing_revisions_lower_status_reasons_test() ->
    ...
    {ok, Output} = rebar3_reltree_fs:atomic_write(Output, Bytes, #{}),
    Failed = Prior#{
        lookup => fun(_Url, _Selector) -> {error, timeout} end,
    ...
    ?assertEqual(
        maps:get(resolved_revision, Good),
...
prior_state_field_combinations_are_strict_test() ->
    ...
    Valid = [
        ...
        {"missing", "file:///repo", "head", "HEAD", "none", "not-performed", Time},
        {"missing", "none", "none", "none", "none", "not-performed", "not-performed"},
        {"tracking-disabled", "file:///repo", "head", "HEAD", "none", "not-performed",
            "not-performed"},
        {"tracking-disabled", "none", "none", "none", "none", "not-performed", "not-performed"},
        {"not-applicable", "none", "none", "none", "none", "not-performed", "not-performed"}
    ],
    lists:foreach(
        fun(Fields) ->
            ?assertMatch(
    ...
    Invalid = [
        ...
        {"reused", "file:///repo", "head", "HEAD", Revision, "not-performed", Time},
        {"stale", "file:///repo", "head", "HEAD", "none", Time, Time},
        {"missing", "file:///repo", "head", "HEAD", Revision, "not-performed", Time},
        {"not-applicable", "file:///repo", "head", "HEAD", "none", "not-performed", "not-performed"}
    ],
    lists:foreach(
        fun(Fields) ->
            ?assertMatch(
...
node(Path, Deps, Relationships) ->
    #{
        ...
        app => app,
        app_vsn => "0.1.0",
        app_src => filename:join(Path, "app.app.src"),
...
parse_record({State, Url, Kind, Value, Revision, Observed, Network}) ->
    rebar3_reltree_rev:parse_prior(
        iolist_to_binary([
            ...
            "; relationship: external; revision_state: ",
            State,
            "\n",
            "    source_url: ",
            Url,
