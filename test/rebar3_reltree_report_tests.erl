...
renderer_uses_only_model_relationship_facts_test() ->
    ...
    Node = Node0#{
        path => "/tmp/missing-renderer-project",
    ...
    {ok, Bytes} = rebar3_reltree_report:render(
        (model())#{
            current => "/tmp/missing-renderer-project",
            nodes => [Node]
        }
    ),
    ?assertMatch(
...
all_atomic_failure_stages_preserve_prior_bytes_test() ->
    lists:foreach(
        fun(Stage) -> atomic_failure_stage(Stage) end,
        [write, close, rename]
    ).
...
declarations_are_sorted_by_name_and_term_test() ->
    ...
    Node = #{
        ...
        app => current,
        app_vsn => "0.1.0",
        app_src => "/tmp/current/src/current.app.src",
        head => "head",
        version => #{
            highest_formal => none,
