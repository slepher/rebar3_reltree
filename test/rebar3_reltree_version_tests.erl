...
formal_tag_filtering_and_highest_version_test() ->
    Facts = rebar3_reltree_version:evaluate(
              "1.2.1", ["v1.2.0", "1.2.1-rc.1", "check-1.2.9",
                         "1.0.0", "1.2.0"]),
    ?assertEqual(#{tag => "v1.2.0", version => {1, 2, 0}},
    ...
    ?assertEqual({formal, {1, 2, 0}, "v1.2.0"},
...
version_status_reasons_test() ->
    ?assertEqual(no_formal_tag,
                 maps:get(reason, rebar3_reltree_version:evaluate(
                                   "9.0.0", []))),
    ?assertEqual(update_required,
                 maps:get(status, rebar3_reltree_version:evaluate(
                                   "1.2.4", ["1.2.0"]))),
    ?assertEqual(insufficient_local_data,
                 maps:get(status, rebar3_reltree_version:evaluate(
                                   "2.0.0", ["1.2.0"]))),
    ?assertEqual(generation_selection_needed,
                 maps:get(reason, rebar3_reltree_version:evaluate(
                                   "2.0.0", ["1.2.0"]))),
    ?assertEqual(prerelease_base_mismatch,
                 maps:get(reason, rebar3_reltree_version:evaluate(
                                  "1.2.0", ["1.2.0", "1.1.0-rc.1"]))).

continuous_patch_and_breaking_lines_are_valid_test() ->
    ?assertEqual(up_to_date,
                 maps:get(status, rebar3_reltree_version:evaluate(
                                   "1.2.1", ["1.2.0"]))),
    ?assertEqual(up_to_date,
                 maps:get(status, rebar3_reltree_version:evaluate(
...
only_release_prerelease_forms_are_accepted_test() ->
    ?assertEqual({prerelease, {1, 2, 3}, "1.2.3-rc.1"},
                 rebar3_reltree_version:parse_tag("1.2.3-rc.1")),
    ?assertEqual({prerelease, {1, 2, 3}, "v1.2.3-ci.2"},
