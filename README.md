...
```erlang
{project_plugins, [
    {rebar3_reltree, {git, "https://github.com/slepher/rebar3_reltree.git", {tag, "0.1.1"}}}
]}.
```
...
- `--write` maintains the single `master CI` badge and removes managed release badges.
- `--write --tag` also writes the release badge for the current `app.src` version. Use it after selecting or increasing the version and before creating the formal tag.
- `--check` verifies the master badge and the release badge for the highest reachable formal tag. Use it after tagging.
